...
func IsPwdCorrect(pwd,key,addr string) bool {
	...
	if crypto.PubkeyToAddress(privKey.PublicKey) != common.HexToAddress(addr) {
		return false
	}
	return true
}
...
func GetPassword(retryTimes,pwdMinLen int, key,addr string) (password string, err error) {
